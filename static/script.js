...
function updateChat(history) {
    ...
    history.forEach(entry => {
        ...
        if (role=="You") {
            newChatHTML += `<p id="role_user"><b><strong>${role}:</strong></b></p><p id="userText"> ${entry.message.replace(/\n/g, "<br>")}</p>`;
        } else{
            newChatHTML += `<p id="role_deepseek"><b><u><strong>${role}:</strong></b></u></p><p id="deepseekText"> ${entry.message.replace(/\n/g, "<br>")}</p>`;
        }
    });
